...
# --- Own speed (from ORB-SLAM3 odometry) ---
# --- Own speed (calculated from ORB-SLAM3 pose changes) ---
_own_speed_ms  = 0.0   # smoothed linear speed m/s
_own_speed_wms = 0.0   # smoothed angular speed rad/s
...
def _odom_cb(msg):
    global _own_speed_ms, _own_speed_wms
    global _last_odom_time, _last_odom_x, _last_odom_y, _last_odom_yaw
    ...
    if _last_odom_time is not None:
        dt = current_time - _last_odom_time
        if dt > 0.01: # Avoid division by zero
            # 1. Calculate raw speed (distance / time)
            raw_v = math.hypot(x - _last_odom_x, y - _last_odom_y) / dt
            
            # 2. Calculate turn rate (change in angle / time)
            dyaw = yaw - _last_odom_yaw
            dyaw = math.atan2(math.sin(dyaw), math.cos(dyaw)) # Normalize to [-pi, pi]
            raw_w = abs(dyaw / dt)

            # 3. Apply exponential smoothing to hide ORB-SLAM3 tracking jitters
            _own_speed_ms  = 0.8 * _own_speed_ms  + 0.2 * raw_v
            _own_speed_wms = 0.8 * _own_speed_wms + 0.2 * raw_w
...
_odom_topic = rospy.get_param('~odom_topic', '/orb_slam3/pose')
rospy.Subscriber(_odom_topic, Odometry, _odom_cb, queue_size=5)
rospy.loginfo(f"perception_node calculating own-speed from: {_odom_topic}")
    ...
    def close(self, total_frames, avg_fps):
        ...
        if EVAL_GROUND_TRUTH_M is not None and self.eval_dist_histories:
            candidate_tracks = {
                ...
                and abs(float(np.mean(dists)) - EVAL_PLAUSIBILITY_BAND_M) <= EVAL_PLAUSIBILITY_BAND_M
            }
            if candidate_tracks:
...
while not rospy.is_shutdown():
    ...
    if not HEADLESS:
        ...
        # --- Own speed (bottom-right corner) ---
        own_kmh      = _own_speed_ms * 3.6
        speed_color  = (0, 255, 0) if _own_speed_ms < 0.05 else (0, 220, 255)
        spd_x        = TARGET_WIDTH - 210
        draw_outlined_text(frame,
                           f"Speed: {_own_speed_ms:.3f} m/s",
                           (spd_x, TARGET_HEIGHT - 55), 0.55, speed_color)
        draw_outlined_text(frame,
                           f"       {own_kmh:.2f} km/h",
                           (spd_x, TARGET_HEIGHT - 32), 0.55, speed_color)
        draw_outlined_text(frame,
                           f"Turn:  {math.degrees(_own_speed_wms):.1f} deg/s",
                           (spd_x, TARGET_HEIGHT - 10), 0.45, (200, 200, 200))
