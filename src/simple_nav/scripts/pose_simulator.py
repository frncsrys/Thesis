...
class PoseSimulator:
    ...
    def _cmd_cb(self, msg: Twist):
        raw_v = clamp(msg.linear.x,  -self.v_max, self.v_max)
        self.w = clamp(msg.angular.z, -self.w_max, self.w_max)
        
        # STRICT PIVOT LOGIC:
        # If the angular command is high (> 0.4 rad/s), force the linear velocity to 0.0.
        # This completely prevents wide circles and forces the robot to rotate in place 
        # until it faces the path.
        if abs(self.w) > 0.4:
            self.v = 0.0
        else:
            self.v = raw_v
    ...
    def _update_cb(self, _event):
        ...
        # Calculate yaw first to allow in-place rotation
        self.yaw += self.w * dt
        # Normalise yaw to (-π, π]
        self.yaw  = math.atan2(math.sin(self.yaw), math.cos(self.yaw))

        # Update position based on the new yaw
